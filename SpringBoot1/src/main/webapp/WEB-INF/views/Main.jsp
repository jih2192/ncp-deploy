...
<body>
	...
	<div id="wrapper">
	    ...
	    <!-- 로그인 전 => 로그인 메뉴 띄우기 -->
		<c:if test="${empty member }"></c:if> <a href="#menu">로그인</a> 
		...
		<div class="inner">
			...
			<!-- ex) smhrd님 환영합니다.  --> <!-- 로그인 수행 전/후에 따른 h1태그 출력을 생각해야 한다! -->
			<!-- 로그인 전이라면 즉, session 영역에 member가 없다면! --> <!-- empty : null과 같은 의미 -->
			<c:if test="${empty member }">
				<h1>로그인 한 세션아이디를 출력해주세요</h1>
			</c:if> <!-- 로그인 후라면 즉, session 영역에 member가 있다면! --> <c:if
